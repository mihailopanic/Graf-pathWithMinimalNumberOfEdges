...
// TODO: Realizovati metod koji vraca da li postoji i odredjuje put u grafu izmedju zadatog 
// pocetnog i krajnjeg cvora cija je duzina (broj potega na putu) najmanja.

bool GraphAsListsInt::pathWithMinimalNumberOfEdges(int dataSource, int dataDest)
{
	LinkedNodeInt* ptr, *first = nullptr, *last = nullptr;
	ptr = start;
	while (ptr != nullptr) {
		ptr->prev = nullptr;
		ptr->status = 0;
		if (ptr->node == dataSource) {
			first = ptr;
		}
		if (ptr->node == dataDest) {
			last = ptr;
		}
		ptr = ptr->next;
	}
	...
	{
		cout << "Put izmedju cvorova " << dataSource << " i " << dataDest << " ne postoji." << endl;
		return false;
	}
	
	if (first == last)
		cout << "Cvorovi " << dataSource << " i " << dataDest << " su zapravo jedan isti cvor." << endl;
		return true;

	if (pathBFS(first, last)) {
		printBFSPath(last);
		return true;
	}
...
bool GraphAsListsInt::pathBFS(LinkedNodeInt* first, LinkedNodeInt* last)
{
	QueueAsArrayLinkedNodeInt queue(nodeNum);
	LinkedNodeInt* ptr = first;
	queue.enqueue(ptr);
	while (!queue.isEmpty()) {
		ptr = queue.dequeue();
		if (ptr == last)
...
void GraphAsListsInt::printBFSPath(LinkedNodeInt* ptr) 
{
	if (ptr != nullptr) 
	{
		printBFSPath(ptr->prev);
		cout << ptr->node << " ";
	}
}
